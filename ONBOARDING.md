...
app/
  ...
  analytics/
    ...
    gru.py                  обучение, валидация и рекурсивный прогноз GRU
  pipeline/
    data_loader.py          загрузка CSV и выбор магазина
    forecasting.py          валидация ряда, split, будущие признаки
...
4. `split_time_series()` делает хронологическое разбиение `train / validation / test`.
5. `run_sarimax_pipeline()`:
   - проверяет стационарность ADF-тестом;
   - подбирает параметры по validation RMSE;
   - обучает финальную модель на `train + validation`;
   - оценивает качество на `test`;
   - отдельно обучается на всей истории для будущего прогноза.
6. `run_gru_pipeline()`:
   - добавляет календарные признаки `Week_Sin` и `Week_Cos`;
   - масштабирует признаки только по train для честной оценки;
   - обучает GRU с early stopping по validation;
   - переобучает финальную модель на `train + validation`;
...
- `TRAIN_RATIO`, `VALIDATION_RATIO`, `TEST_RATIO` - доли хронологического разбиения, сумма должна быть `1.0`.
...
- Валидация используется для выбора параметров, тест не участвует в подборе.
- SARIMAX подбирает `order` и `seasonal_order` по validation RMSE.
- GRU подбирает число эпох через early stopping на validation.
...
Важно: будущие погодные и экономические признаки для GRU не прогнозируются отдельными моделями. Они берутся как среднее по последнему окну, а `Holiday_Flag` вычисляется по известным праздничным неделям Walmart.
