...
def _metrics_rows(report: ComparisonReport) -> list[tuple[str, str, str, str]]:
    rows: list[tuple[str, str, str, str]] = []
    for period_label, sarimax, gru in [
        ("Обучение", report.sarimax.training_metrics, report.gru.training_metrics),
        ("Тест", report.sarimax.test_metrics, report.gru.test_metrics),
    ]:
        rows.extend(
            [
                (period_label, "MAE", f"{sarimax.mae:.2f}", f"{gru.mae:.2f}"),
                (period_label, "MSE", f"{sarimax.mse:.2f}", f"{gru.mse:.2f}"),
                (period_label, "RMSE", f"{sarimax.rmse:.2f}", f"{gru.rmse:.2f}"),
                (period_label, "MAPE, %", f"{sarimax.mape:.2f}", f"{gru.mape:.2f}"),
                (period_label, "sMAPE, %", f"{sarimax.smape:.2f}", f"{gru.smape:.2f}"),
                (period_label, "WAPE, %", f"{sarimax.wape:.2f}", f"{gru.wape:.2f}"),
            ],
        )
    return rows
...
def _plot_overview_page(pdf: PdfPages, report: ComparisonReport) -> None:
    ...
    data_summary = report.data_validation
    split = report.split
    axis.text(
        0.02,
        0.95,
    ...
    axis.text(
        ...
        (
            ...
            f"Количество строк: {data_summary.row_count}\n"
            f"Период: {data_summary.start_date} - {data_summary.end_date}\n"
            f"Частота: {data_summary.frequency_days} дней\n"
            f"Пропуски: {data_summary.missing_values}\n"
            f"Дубликаты дат: {data_summary.duplicated_dates}\n"
    ...
    axis.text(
        ...
        (
            ...
            f"Тестирование: {split.test_size} ({split.test_start} - {split.test_end})\n"
            f"Прогнозирование: {report.horizon} недель после {data_summary.end_date}"
        ),
        fontsize=9,
        va="top",
    )
    axis.text(
        ...
        (
            ...
            f"GRU эпохи обучения: {report.gru.hyperparams.selected_epochs}\n"
    ...
    table = axis.table(
        ...
        colLabels=["Период", "Метрика", "SARIMAX", "GRU"],
        cellLoc="center",
        loc="lower center",
        bbox=[0.06, 0.08, 0.88, 0.44],
    )
...
def _plot_error_page(pdf: PdfPages, report: ComparisonReport) -> None:
    ...
    fig, axis = plt.subplots(figsize=(11.69, 8.27))
    axis.plot(dates, sarimax_abs_error, label="Абсолютная ошибка SARIMAX", color="#2563eb")
    axis.plot(dates, gru_abs_error, label="Абсолютная ошибка GRU", color="#dc2626")
    axis.set_title("Абсолютная ошибка на тестовом периоде", fontsize=11)
    axis.set_xlabel("Дата", fontsize=9)
    axis.set_ylabel("Абсолютная ошибка", fontsize=9)
    axis.tick_params(axis="both", labelsize=8)
    axis.grid(alpha=0.3)
    axis.legend(fontsize=8)
...
def _correlation_lags(sample_size: int) -> int | None:
    if sample_size < 4:
        return None
    return min(20, sample_size // 2 - 1)


def _plot_correlation_or_message(
    axis,
    values: pd.Series,
    title: str,
    kind: str,
) -> None:
    lags = _correlation_lags(len(values))
    if lags is None:
        axis.text(0.5, 0.5, "Недостаточно точек для расчета", ha="center", va="center")
        axis.set_title(title, fontsize=10)
        axis.axis("off")
        return

    if kind == "acf":
        plot_acf(values, lags=lags, ax=axis, zero=False)
    else:
        plot_pacf(values, lags=lags, ax=axis, method="ywm", zero=False)
    axis.set_title(title, fontsize=10)
    axis.tick_params(axis="both", labelsize=8)
    axis.grid(alpha=0.25)


def _plot_residual_diagnostics_page(pdf: PdfPages, report: ComparisonReport) -> None:
    sarimax_df = pd.DataFrame([point.model_dump() for point in report.sarimax.test_forecast])
    gru_df = pd.DataFrame([point.model_dump() for point in report.gru.test_forecast])
    sarimax_errors = sarimax_df["actual"] - sarimax_df["predicted"]
    gru_errors = gru_df["actual"] - gru_df["predicted"]

    fig, axes = plt.subplots(2, 2, figsize=(11.69, 8.27))
    fig.suptitle("ACF и PACF погрешностей на тестовом периоде", fontsize=13, weight="bold")

    _plot_correlation_or_message(
        axes[0][0],
        sarimax_errors,
        "SARIMAX: ACF погрешностей",
        "acf",
    )
    _plot_correlation_or_message(
        axes[0][1],
        sarimax_errors,
        "SARIMAX: PACF погрешностей",
        "pacf",
    )
    _plot_correlation_or_message(
        axes[1][0],
        gru_errors,
        "GRU: ACF погрешностей",
        "acf",
    )
    _plot_correlation_or_message(
        axes[1][1],
        gru_errors,
        "GRU: PACF погрешностей",
        "pacf",
    )
...
def generate_comparison_report(
    ...
    validated_df, data_summary = validate_store_dataframe(store_df, min_rows=min_rows)
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        _plot_residual_diagnostics_page(pdf, report)
